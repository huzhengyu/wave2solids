...
defineTypeNameAndDebug(wave2solids, 0);
addToRunTimeSelectionTable(physicsModel, wave2solids, fluid);
addToRunTimeSelectionTable(fluidModel, wave2solids, dictionary);
...
tmp<vectorField> wave2solids::patchViscousForce(const label patchID) const
{
    tmp<vectorField> tvF
    (
...
tmp<scalarField> wave2solids::patchPressureForce(const label patchID) const
{
    tmp<scalarField> tpF
    (
